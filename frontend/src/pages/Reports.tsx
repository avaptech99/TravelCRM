...
const StatCard: React.FC<StatCardProps> = ({ title, value, icon, trend, loading }) => (
    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm hover:shadow-lg transition-all duration-300">
        <div className="flex items-center justify-between mb-4">
            ...
            {trend && (
                <div className={`flex items-center gap-1 text-[10px] font-black uppercase tracking-widest ${trend.isPositive ? 'text-emerald-600' : 'text-red-600'}`}>
    ...
    return (
        <div className="space-y-8 pb-12">
                ...
                <div className="flex items-center gap-3 bg-white p-2.5 rounded-2xl border border-slate-200 shadow-sm">
                    <div className="flex items-center gap-2">
                        ...
                        <input
                            ...
                            className="text-sm font-bold border-none focus:ring-0 text-slate-800 bg-transparent p-1 cursor-pointer"
                        />
                        ...
                        <input
                            ...
                            className="text-sm font-bold border-none focus:ring-0 text-slate-800 bg-transparent p-1 cursor-pointer mr-2"
                        />
                    </div>
                </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 px-2">
                ...
                <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm h-[450px] flex flex-col hover:shadow-md transition-shadow">
                        ...
                        {isAgentsLoading ? (
                            ...
                            <ResponsiveContainer width="100%" height="100%">
                                <BarChart data={agentStats?.slice(0, 5)} layout="vertical" margin={{ left: 20 }}>
                                    ...
                                    <YAxis dataKey="agentName" type="category" axisLine={false} tickLine={false} tick={{fontSize: 11, fontWeight: 800, fill: '#334155'}} width={100} />
                                    <Tooltip 
                                        cursor={{fill: '#f8fafc'}}
                                        contentStyle={{ borderRadius: '16px', border: 'none', boxShadow: '0 20px 25px -5px rgb(0 0 0 / 0.1)', padding: '12px' }}
                                    />
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 px-2">
                ...
                <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm h-[400px] flex flex-col">
                    <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6">Booking Pipeline Status</h3>
                    <div className="flex-1 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie
                                ...
                                <Tooltip />
                                <Legend verticalAlign="bottom" height={36} iconType="circle" wrapperStyle={{ fontWeight: 800, fontSize: '10px', textTransform: 'uppercase' }}/>
                            </PieChart>
                        </ResponsiveContainer>
                    </div>
                </div>
                ...
                {/* Interest Level */}
                <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm h-[400px] flex flex-col">
                    <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6">Lead Interest Conversion</h3>
                    <div className="flex-1 w-full">
                        <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                                <Pie
                ...
                <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-sm flex flex-col lg:col-span-1">
                    <h3 className="text-xs font-black text-slate-400 uppercase tracking-widest mb-6">Conversion Efficiency</h3>
                            ...
                            <tbody className="divide-y divide-slate-50">
                                {agentStats?.map((agent: any) => (
                                    <tr key={agent._id} className="hover:bg-slate-50/50 transition-colors group">
                                        <td className="py-4 font-bold text-slate-700">{agent.agentName}</td>
