...
import { Plus, Trash2, Calendar, Plane, CreditCard, ArrowLeft, Users, FileText, List, ChevronDown, CheckCircle2, Building2, Lock } from 'lucide-react';
...
export const BookingTravelers: React.FC = () => {
    ...
    const { user } = useAuth();
    const canEditActualCosts = user?.role === 'ADMIN' || user?.role === 'OPERATION';
    ...
    const [actualLumpSumAmount, setActualLumpSumAmount] = useState<number>(0);
            ...
            <div className="flex items-center space-x-4 mb-6 sm:mb-8">
                ...
                <div>
                    <h1 className="text-2xl font-bold text-slate-900">
                        Finalize Booking <span className="text-primary ml-2">#{booking?.uniqueCode}</span>
                    </h1>
                    {/* @ts-ignore */}
                                        ...
                                        >
                                            <span>
                                                {finalQuotationAmount 
                                                    ? (finalQuotationAmount.length === 1 && booking?.uniqueCode 
                                                        ? `${booking.uniqueCode}-${finalQuotationAmount}` 
                                                        : finalQuotationAmount)
                                                    : 'Select quotation...'}
                                            </span>
                        ...
                        <div className="mt-6 pt-6 border-t border-slate-100">
                            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 md:gap-6">
                                <div className="flex items-center gap-4">
                                    ...
                                    <h2 className="text-xl font-medium text-slate-900">Company / Agency</h2>
                                </div>
                                <div className="relative min-w-[240px]">
                                    <DropdownMenu>
                                        <DropdownMenuTrigger asChild>
                                            <button 
                                                type="button"
                                                className="w-full px-4 py-3 bg-slate-50 border-2 border-slate-100 rounded-xl focus:outline-none focus:ring-4 focus:ring-primary/10 focus:border-primary text-lg font-medium text-slate-800 transition-all shadow-inner cursor-pointer flex items-center justify-between group hover:border-primary/30"
                                            >
                                        ...
                                        <DropdownMenuContent className="w-[var(--radix-dropdown-menu-trigger-width)] max-h-[300px] overflow-y-auto bg-white border border-slate-200 rounded-xl shadow-xl z-50">
                                            {(dropdownSettings?.companies || []).map(opt => (
                                                <DropdownMenuItem 
                                                    key={opt} 
            ...
            {/* 2. TRAVELERS SECTION */}
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-6">
                    ...
                    <div className={`transition-all duration-300 ease-in-out ${isTravelerDetailsExpanded ? 'max-h-[10000px] opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                        ...
                        >
                            <span className="flex items-center gap-2"><Plus size={18} /> Add Another Passenger</span>
                        </button>
                    </div>
                ...
                <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden mb-6">
                    ...
                    >
                        <div className="flex items-center gap-2">
                            ...
                            <h2 className="text-base font-semibold text-slate-800">Pricing Information (Est. vs Actual)</h2>
                        </div>
                        <div className="flex items-center gap-4">
                            {!isPricingExpanded && lumpSumAmount > 0 && (
                                <div className="flex items-center gap-1.5 bg-blue-50 px-3 py-1 rounded-md border border-blue-200">
                                    <span className="text-blue-700 text-[10px] font-bold uppercase tracking-wider">Est:</span>
                                    <span className="text-blue-800 font-semibold text-sm">{lumpSumAmount.toLocaleString()}</span>
                                </div>
                            )}
                            {!isPricingExpanded && actualLumpSumAmount > 0 && (
                                <div className="flex items-center gap-1.5 bg-emerald-50 px-3 py-1 rounded-md border border-emerald-200">
                                    <span className="text-emerald-700 text-[10px] font-bold uppercase tracking-wider">Act:</span>
                                    <span className="text-emerald-800 font-semibold text-sm">{actualLumpSumAmount.toLocaleString()}</span>
                                </div>
                            )}
                            <ChevronDown 
                                size={20} 
                    ...
                    <div className={`transition-all duration-300 ease-in-out ${isPricingExpanded ? 'max-h-[8000px] opacity-100' : 'max-h-0 opacity-0 overflow-hidden'}`}>
                        <div className="p-5">
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                {/* LEFT SIDE: ESTIMATED COST */}
                                <div className="w-full bg-blue-50/30 rounded-2xl border border-blue-100 p-6 shadow-sm">
                                    <div className="flex items-center gap-3 mb-6">
                                        <div className="bg-blue-600 p-2 rounded-lg text-white shadow-md">
                                            <FileText size={18} />
                                        </div>
                                        <h3 className="text-lg font-bold text-blue-900">Estimated Cost</h3>
                                    </div>

                                    <div className="space-y-6">
                                        <div>
                                            <label className="block text-[10px] font-black text-blue-600 uppercase tracking-widest mb-3">Total Estimated Amount</label>
                                            <div className="relative">
                                                <input
                                                    type="number"
                                                    min="0"
                                                    ...
                                                    className="w-full px-5 py-4 bg-white border-2 border-blue-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-blue-100 focus:border-blue-500 text-xl font-black text-slate-800 transition-all shadow-inner [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none"
                                                    placeholder="0.00"
                                                />
                                            </div>
                                        </div>
                                        ...
                                        <div className="pt-6 border-t border-blue-100">
                                            <div className="mb-6">
                                                <label className="block text-[10px] font-bold text-blue-500 uppercase tracking-widest mb-3">Select Cost Types</label>
                                                <div className="flex flex-wrap gap-2">
                                                    {(dropdownSettings?.costTypes || ['Air Ticket', 'Hotel', 'Visa']).map(type => {
                                            ...
                                            {estimatedCosts.length > 0 && (
                                                <div className="space-y-4">
                                                    {estimatedCosts.map((cost, index) => (
                                                        <div key={cost.costType} className="grid grid-cols-1 gap-4 p-4 bg-white border border-blue-100 rounded-xl shadow-sm">
                                                            <div className="flex items-center justify-between">
                                                                <span className="text-[10px] font-black text-blue-600 uppercase">{cost.costType}</span>
                                                            </div>
                                                            <div className="grid grid-cols-2 gap-3">
                                                                <div>
                                                                    <label className="block text-[8px] font-bold text-slate-400 uppercase mb-1">Price</label>
                                                                    <input
                                                                        type="number"
                                                                        value={cost.price || ''}
                                                                        onChange={(e) => {
                                                                        ...
                                                                        className="w-full px-3 py-2 bg-slate-50 border border-blue-100 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-400 text-sm font-bold"
                                                                        placeholder="0.00"
                                                                    />
                                                                </div>
                                                                <div>
                                                                    <label className="block text-[8px] font-bold text-slate-400 uppercase mb-1">Source</label>
                                                                    <select
                                                                        value={cost.source || ''}
                                                                        onChange={(e) => {
                                                                            const newCosts = [...estimatedCosts];
                                                                        ...
                                                                        className="w-full px-3 py-2 bg-slate-50 border border-blue-100 rounded-lg focus:ring-2 focus:ring-blue-200 focus:border-blue-400 text-xs font-semibold h-[36px]"
                                                                    >
                                                                        <option value="">Select</option>
                                                                        {(dropdownSettings?.costSources || []).map(s => (
                                            ...
                                            <div className="mt-8 pt-6 border-t border-blue-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                                                <button
                                                    ...
                                                    onClick={() => {
                                                        ...
                                                        toast.success('Estimated margin calculated');
                                                    }}
                                                    className="w-full sm:w-auto flex items-center gap-2 px-5 py-3 bg-blue-600 text-white rounded-xl font-bold text-xs hover:bg-blue-700 transition-all shadow-md active:scale-95"
                                                >
                                                    <CreditCard size={14} />
                                                    Calculate Margin
                                                </button>

                                                <div className="flex items-center gap-3 bg-white px-4 py-3 rounded-xl border border-blue-100 shadow-inner">
                                                    <span className="text-[9px] font-black text-blue-400 uppercase tracking-widest">Est. Margin</span>
                                                    <span className={`text-base font-black ${estimatedMargin >= 0 ? 'text-blue-700' : 'text-red-600'}`}>
                                ...
                                {/* RIGHT SIDE: ACTUAL COST */}
                                <div className="w-full bg-emerald-50/30 rounded-2xl border border-emerald-100 p-6 shadow-sm relative overflow-hidden">
                                    <div className="flex items-center gap-3 mb-6">
                                        <div className="bg-emerald-600 p-2 rounded-lg text-white shadow-md">
                                            <CheckCircle2 size={18} />
                                        </div>
                                        <h3 className="text-lg font-bold text-emerald-900">Actual Cost</h3>
                                        {!canEditActualCosts && (
                                            <span className="ml-auto flex items-center gap-1 text-[10px] font-bold text-emerald-600 bg-emerald-100 px-2 py-1 rounded-md">
                                    ...
                                    <div className="space-y-6">
                                        <div>
                                            <label className="block text-[10px] font-black text-emerald-600 uppercase tracking-widest mb-3">Total Actual Amount</label>
                                            <div className="relative">
                                                <input
                                                    type="number"
                                                    min="0"
                                                    step="0.01"
                                                    disabled={!canEditActualCosts}
                                                    value={actualLumpSumAmount || ''}
                                                    onChange={(e) => setActualLumpSumAmount(parseFloat(e.target.value) || 0)}
                                                    onWheel={(e) => (e.target as HTMLElement).blur()}
                                                    className="w-full px-5 py-4 bg-white border-2 border-emerald-200 rounded-xl focus:outline-none focus:ring-4 focus:ring-emerald-100 focus:border-emerald-500 text-xl font-black text-slate-800 transition-all shadow-inner [appearance:textfield] [&::-webkit-outer-spin-button]:appearance-none [&::-webkit-inner-spin-button]:appearance-none disabled:bg-slate-50 disabled:border-slate-100 disabled:text-slate-400"
                                                    placeholder="0.00"
                                                />
                                            </div>
                                        </div>
                                        ...
                                        <div className="pt-6 border-t border-emerald-100">
                                            <div className="mb-6">
                                                <label className="block text-[10px] font-bold text-emerald-500 uppercase tracking-widest mb-3">Select Cost Types</label>
                                                <div className="flex flex-wrap gap-2">
                                                    {(dropdownSettings?.costTypes || ['Air Ticket', 'Hotel', 'Visa']).map(type => {
                                                        const isActive = actualCosts.some(c => c.costType === type);
                                                        return (
                                                            <button
                                                                key={type}
                                                                type="button"
                                                                ...
                                                                onClick={() => {
                                                                    if (isActive) {
                                                                        setActualCosts(prev => prev.filter(c => c.costType !== type));
                                                                    } else {
                                                                        setActualCosts(prev => [...prev, { costType: type, price: 0, source: '' }]);
                                                                    }
                                                                }}
                                                                className={`px-4 py-2 rounded-full text-[10px] font-black transition-all border-2 ${
                                                                    isActive
                                            ...
                                            {actualCosts.length > 0 && (
                                                <div className="space-y-4">
                                                    {actualCosts.map((cost, index) => (
                                                        <div key={cost.costType} className="grid grid-cols-1 gap-4 p-4 bg-white border border-emerald-100 rounded-xl shadow-sm">
                                                            <div className="flex items-center justify-between">
                                                                <span className="text-[10px] font-black text-emerald-600 uppercase">{cost.costType}</span>
                                                            </div>
                                                            <div className="grid grid-cols-2 gap-3">
                                                                <div>
                                                                    <label className="block text-[8px] font-bold text-slate-400 uppercase mb-1">Price</label>
                                                                    <input
                                                                        ...
                                                                        className="w-full px-3 py-2 bg-slate-50 border border-emerald-100 rounded-lg focus:ring-2 focus:ring-emerald-200 focus:border-emerald-400 text-sm font-bold disabled:opacity-50"
                                                                        placeholder="0.00"
                                                                    />
                                                                </div>
                                                                <div>
                                                                    ...
                                                                    <select
                                                                        ...
                                                                        className="w-full px-3 py-2 bg-slate-50 border border-emerald-100 rounded-lg focus:ring-2 focus:ring-emerald-200 focus:border-emerald-400 text-xs font-semibold h-[36px] disabled:opacity-50"
                                                                    >
                                                                        <option value="">Select</option>
                                                                        {(dropdownSettings?.costSources || []).map(s => (
                                                                            <option key={s} value={s}>{s}</option>
                                            ...
                                            <div className="mt-8 pt-6 border-t border-emerald-100 flex flex-col sm:flex-row items-center justify-between gap-4">
                                                <button
                                                    ...
                                                    disabled={!canEditActualCosts}
                                                    onClick={() => {
                                                        const totalCost = actualCosts.reduce((sum, c) => sum + (parseFloat(c.price?.toString() || '0')), 0);
                                                        const salePrice = actualLumpSumAmount || lumpSumAmount || 0;
                                                        setActualMargin(salePrice - totalCost);
                                                        toast.success('Actual margin calculated');
                                                    }}
                                                    className="w-full sm:w-auto flex items-center gap-2 px-5 py-3 bg-emerald-600 text-white rounded-xl font-bold text-xs hover:bg-emerald-700 transition-all shadow-md active:scale-95 disabled:opacity-50 disabled:scale-100"
                                                >
                                                    <CreditCard size={14} />
                                                    Calculate Margin
                                                </button>

                                                <div className="flex items-center gap-3 bg-white px-4 py-3 rounded-xl border border-emerald-100 shadow-inner">
                                                    <span className="text-[9px] font-black text-emerald-400 uppercase tracking-widest">Act. Margin</span>
                                                    <span className={`text-base font-black ${actualMargin >= 0 ? 'text-emerald-700' : 'text-red-600'}`}>
                                                        {actualMargin.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                                    </span>
                                                </div>
                                            </div>
                                        </div>
