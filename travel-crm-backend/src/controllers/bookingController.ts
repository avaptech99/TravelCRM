...
export const createBooking = asyncHandler(async (req: Request, res: Response) => {
    ...
    const booking = await Booking.create({
        ...
        participantIds: req.user?.id ? [new mongoose.Types.ObjectId(req.user.id)] : [],
    ...
    setImmediate(() => runBG(`createBooking_sideEffects_${(booking as any)._id}`, async () => {
        await Promise.all([
...
export const updateBooking = asyncHandler(async (req: Request, res: Response) => {
    ...
    setImmediate(() => runBG(`updateBooking_sideEffects_${id}`, async () => {
        ...
        if ((updatedBooking as any).primaryContactId) {
            ...
            await PrimaryContact.findByIdAndUpdate((updatedBooking as any).primaryContactId, legacyUpdate);
        }
    }));
});
...
export const addPayment = asyncHandler(async (req: Request, res: Response) => {
    ...
    const payment = await Payment.create({ ...result.data, bookingId: id });
    await recalcOutstanding(id);
    appCache.del(`booking_${id}`);
    invalidateBookingCaches();
...
export const deletePayment = asyncHandler(async (req: Request, res: Response) => {
    ...
    await Payment.deleteOne({ _id: paymentId, bookingId: id });
    await recalcOutstanding(id);
    appCache.del(`booking_${id}`);
    invalidateBookingCaches();
...
export const getCalendarBookings = asyncHandler(async (req: Request, res: Response) => {
    ...
    const query: any = { travelDate: { $gte: startDate, $lte: endDate } };
    if (req.user?.role === 'AGENT') query.assignedToUserId = req.user.id;

    const bookings = await Booking.find(query).select('uniqueCode status destination travelDate contact').lean();
...
export const getBookingActivity = asyncHandler(async (req: Request, res: Response) => {
    const { id } = req.params;
    const activities = await Comment.find({ bookingId: id, type: 'activity' })
