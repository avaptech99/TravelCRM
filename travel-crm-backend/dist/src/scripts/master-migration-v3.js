...
async function runMigration() {
    ...
    try {
        ...
        await db.collection('bookings').createIndex({ assignedToUserId: 1, status: 1, lastInteractionAt: -1 }, { background: true, name: "idx_agent_dashboard_v3" });
        await db.collection('bookings').createIndex({ status: 1, travelDate: 1 }, { background: true, name: "idx_status_traveldate" });
        await db.collection('bookings').createIndex({ primaryContactId: 1, createdAt: -1 }, { background: true, name: "idx_contact_timeline" });
        await db.collection('bookings').createIndex({ createdByUserId: 1, createdAt: -1 }, { background: true, name: "idx_creator_timeline" });
