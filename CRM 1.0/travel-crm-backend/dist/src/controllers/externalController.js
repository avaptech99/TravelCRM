...
exports.createExternalLead = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    // ============================================================
    const rawFields = req.body.raw_fields || [];
    let contactPerson = '';
    let contactNumber = '';
    let contactEmail = '';
    let flightFrom = '';
    ...
    if (rawFields.length > 0) {
        ...
        for (const field of rawFields) {
            ...
            if (!val)
                continue;
            if (typeof val === 'string' && lLow.includes('submit'))
                continue;
            ...
            if (typeof val === 'object' && val !== null && !Array.isArray(val)) {
                ...
                for (const [k, v] of Object.entries(val)) {
                    ...
                    if (typeof v === 'object' && v !== null && 'value' in v) {
                        extractedVal = String(v.value).trim();
                    }
                ...
                for (const row of val) {
                    if (typeof row === 'object' && row !== null) {
                        ...
                        for (const [k, v] of Object.entries(row)) {
                            if (k.endsWith('_id') || k.includes('_id_'))
                                continue;
            ...
            else if (lLow === 'departure') {
                travelDate = strVal;
            }
            else if (lLow === 'return' || lLow.includes('return date') || lLow.includes('return')) {
                returnDate = strVal;
            }
            else if (lLow.includes('adult')) {
                adults = parseInt(strVal) || 0;
            }
            else if (lLow.includes('child')) {
                children = parseInt(strVal) || 0;
            }
            else if (lLow.includes('infant')) {
                infants = parseInt(strVal) || 0;
            }
            else if (lLow.includes('class')) {
                travelClass = strVal;
            }
            else if (lLow.includes('radio') || lLow.includes('trip')) {
                tripType = strVal;
            }
            else if (lLow.includes('email')) {
                contactEmail = strVal;
            }
            else if (lLow.includes('phone') || lLow.includes('mobile')) {
                contactNumber = strVal;
            }
            else if (lLow.includes('name') && !lLow.includes('user')) {
