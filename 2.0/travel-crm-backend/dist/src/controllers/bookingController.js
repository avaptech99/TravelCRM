...
exports.getBookingById = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    const activities = await Activity_1.default.find({ bookingId: id })
...
exports.verifyBooking = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    booking.verifiedBy = new mongoose_1.default.Types.ObjectId(req.user.id);
    await booking.save();
    invalidateBookingCaches();
...
exports.unverifyBooking = (0, express_async_handler_1.default)(async (req, res) => {
    const { id } = req.params;
    const booking = await Booking_1.default.findById(id);
    if (!booking) {
        res.status(404);
...
exports.updateBooking = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    if (result.data.amount !== undefined && result.data.amount !== booking.amount)
        changes.push(`Price: ${booking.amount} ➔ ${result.data.amount}`);
    if (result.data.travelDate !== undefined) {
        ...
        if (oldDate !== newDate)
            changes.push(`Travel Date: ${oldDate} ➔ ${newDate}`);
    }
    ...
    if (changes.length > 0) {
        await (0, activityLogger_1.logActivity)(id, req.user.id, 'BOOKING_UPDATED', `Modified: ${changes.join(', ')}`);
    }
    await booking.save();
...
exports.updateBookingStatus = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    // Log activity
    await (0, activityLogger_1.logActivity)(id, req.user.id, 'STATUS_CHANGE', `Status updated from ${existingBooking.status} to ${status}`);
...
exports.assignBooking = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    if (previousAssignedUserId !== newAssignedUserId) {
        ...
        await (0, activityLogger_1.logActivity)(id, req.user.id, 'ASSIGNED', actionText);
        if (newAssignedUserId) {
            await Notification_1.default.create({
...
exports.bulkAssign = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    const updatePromises = bookings.map(async (booking) => {
        ...
        if (previousAssignedUserId !== (newAgentId ? newAgentId.toString() : null)) {
            ...
            const actionText = `Agent changed: ${previousAgentName} ➔ ${newAgentName}`;
            await (0, activityLogger_1.logActivity)(booking._id, req.user.id, 'ASSIGNED', actionText);
            if (newAgentId) {
                await Notification_1.default.create({
                    userId: newAgentId,
...
exports.addPassengers = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    await (0, activityLogger_1.logActivity)(id, req.user.id, 'PASSENGERS_ADDED', `Added ${passengersData.length} passenger details.`);
    invalidateBookingCaches();
...
exports.addPayment = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    await (0, activityLogger_1.logActivity)(id, req.user.id, 'PAYMENT_RECORDED', `Payment of ${result.data.amount} recorded via ${result.data.paymentMethod}.`);
    await recalcOutstanding(id);
    invalidateBookingCaches();
