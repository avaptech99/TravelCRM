    ...
    return (
        <div className="space-y-8 pb-12">
            ...
            <div className="space-y-6 px-2">
                    ...
                    {showPending && (
                        <div className="border-t border-slate-100 overflow-x-auto">
                            {isBreakdownLoading ? (
                                ...
                                <div className="p-8 text-center text-slate-400 text-sm">No pending payments found</div>
                            ) : (
                                <table className="w-full text-sm">
                                    <thead>
                    ...
                    {showReceived && (
                        <div className="border-t border-slate-100 overflow-x-auto">
                            {isBreakdownLoading ? (
                                ...
                                <div className="p-8 text-center text-slate-400 text-sm">No received payments found</div>
                            ) : (
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100 bg-slate-50/50">
                ...
                {/* Grouped Queries / Duplicate Detection */}
                <GroupedQueries filters={filters} />
            </div>
        </div>
    );
};

const GroupedQueries: React.FC<{ filters: any }> = ({ filters }) => {
    const [isOpen, setIsOpen] = useState(false);
    
    const { data: groupedQueries, isLoading } = useQuery({
        queryKey: ['analytics-grouped-queries', filters],
        queryFn: async () => {
            const { data } = await api.get('/analytics/grouped-queries', { params: filters });
            return data;
        },
    });
        ...
        <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden">
            ...
            >
                <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                        <h3 className="text-sm font-bold text-slate-800">Grouped Queries (by Contact Number)</h3>
                        <p className="text-xs text-slate-500">Detecting potential duplicate leads and recurring clients</p>
                    </div>
                </div>
            ...
            {isOpen && (
                <div className="border-t border-slate-100 overflow-x-auto">
                    {isLoading ? (
                        <div className="p-8 text-center text-slate-400"><Loader2 size={20} className="animate-spin mx-auto" /></div>
                    ) : (groupedQueries?.length || 0) === 0 ? (
                        <div className="p-8 text-center text-slate-400 text-sm">No queries found</div>
                    ) : (
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="text-[10px] font-bold text-slate-400 uppercase tracking-widest border-b border-slate-100 bg-slate-50/50">
                                    <th className="text-left p-4">Contact Number</th>
                                    <th className="text-left p-4">Latest Name</th>
                                    <th className="text-center p-4">Query Count</th>
                                    <th className="text-left p-4">Query Details</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-50">
                                {groupedQueries.map((g: any) => (
                                    <tr key={g.phone} className="hover:bg-slate-50/50 transition-colors">
                                        <td className="p-4 font-semibold text-slate-700">{g.phone}</td>
                                        <td className="p-4 text-slate-600">{g.name}</td>
                                        <td className="p-4 text-center">
                                            <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${g.count > 1 ? 'bg-red-100 text-red-700' : 'bg-slate-100 text-slate-600'}`}>
                                                {g.count}
                                            </span>
                                        </td>
                                        <td className="p-4">
                                            <div className="flex flex-wrap gap-2">
                                                {g.queries.map((q: any) => (
                                                    <span key={q.code} className="inline-flex items-center px-2 py-0.5 rounded bg-slate-50 text-[10px] font-medium text-slate-500 border border-slate-100" title={q.status}>
                                                        {q.code}
                                                    </span>
                                                ))}
                                            </div>
